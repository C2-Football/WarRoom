...
function WrCommishGenesisPanel({ readiness, onToggle }) {
    ...
    return (
        <Section title="Season Setup" meta={meta}>
            ...
            {!list ? (
                <div style={{ color: TEXT, fontSize: 'var(--co-readable-body, 0.78rem)' }}>The readiness scan hasn't run yet — it fills in once your commissioned leagues sync.</div>
            ) : !list.length ? (
                <div style={{ color: TEXT, fontSize: 'var(--co-readable-body, 0.78rem)' }}>No commissioned leagues to score — connect a league you run and the wall lights up.</div>
            ) : allReady ? (
                    ...
                    <div key={r.leagueId} style={{ background: 'var(--black, #121217)', border: `1px solid ${LINE}`, borderRadius: 'var(--card-radius-sm, 8px)', marginBottom: i === list.length - 1 ? 0 : '10px' }}>
                        <button onClick={() => flip(r.leagueId)} aria-expanded={isOpen}
                            ...
                            <span style={{ flex: 1, minWidth: 0 }}>
                                <span style={{ display: 'block', fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 'var(--co-readable-body, 0.82rem)', color: TEXT, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.leagueName}</span>
                                {needs ? (
                                    <span style={{ display: 'block', fontSize: 'var(--co-readable-small, 0.7rem)', color: MUTED, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: '1px' }}>{needs}</span>
                                ) : (
                                    <span style={{ display: 'block', fontSize: 'var(--co-readable-small, 0.7rem)', color: MUTED, marginTop: '1px' }}>Opening-day ready.</span>
                                )}
                            </span>
                            <span style={{ ...mono, fontSize: 'var(--co-readable-small, 0.72rem)', color: MUTED, flexShrink: 0 }}>{isOpen ? '▾' : '▸'}</span>
                        </button>
                        {isOpen ? (
                                    ...
                                    <div key={it.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '9px', padding: '7px 0', borderBottom: ii === items.length - 1 ? 'none' : `1px solid ${LINE}` }}>
                                        {it.kind === 'manual' ? (
                                            <button onClick={() => { if (typeof onToggle === 'function') onToggle(r.leagueId, it.id); }}
                                                ...
                                                style={{ width: '15px', height: '15px', flexShrink: 0, marginTop: '1px', background: 'transparent', border: '1px solid ' + (it.done ? 'rgba(212,175,55,0.7)' : LINE), borderRadius: '3px', color: GOLD, font: '700 var(--co-readable-small, 0.6rem) ' + MONO, lineHeight: '13px', padding: 0, cursor: 'pointer' }}>
                                                {it.done ? '✓' : ''}
                                            </button>
                                        ) : (
                                        ...
                                        <span style={{ flex: 1, minWidth: 0 }}>
                                            <span style={{ display: 'block', fontSize: 'var(--co-readable-body, 0.78rem)', fontWeight: 600, fontFamily: 'var(--font-body)', color: it.done ? SILVER : TEXT }}>{it.label}</span>
                                            {it.detail ? <span style={{ display: 'block', fontSize: 'var(--co-readable-small, 0.7rem)', color: MUTED, lineHeight: 1.45, marginTop: '1px' }}>{it.detail}</span> : null}
                                        </span>
...
function WrCommishRenewalPanel({ forecast }) {
    ...
    return (
        <Section title="Renewal Forecast" meta={forecast ? (offseason ? 'behavior signals' : 'behavior + season arc') : null}>
            {!forecast || !people ? (
                <div style={{ color: TEXT, fontSize: 'var(--co-readable-body, 0.78rem)' }}>The forecast hasn't run yet — it builds from the member graph and the radar sweep.</div>
            ) : !people.length ? (
                <div style={{ color: TEXT, fontSize: 'var(--co-readable-body, 0.78rem)' }}>Nobody to forecast — the member graph only has you in it so far.</div>
            ) : (
                <React.Fragment>
                    {offseason ? (
                        <div style={{ color: TEXT, fontSize: 'var(--co-readable-small, 0.76rem)', fontStyle: 'italic', lineHeight: 1.5, marginBottom: '10px' }}>
                        ...
                        return (
                            <div key={p.userId} style={{ padding: '9px 0', borderBottom: i === people.length - 1 ? 'none' : `1px solid ${LINE}` }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                                    <span style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 'var(--co-readable-body, 0.82rem)', color: TEXT }}>{p.name}</span>
                                    {p.leagueCount ? <span style={{ ...mono, fontSize: 'var(--co-readable-small, 0.68rem)', color: MUTED }}>{p.leagueCount} league{p.leagueCount === 1 ? '' : 's'}</span> : null}
                                    <span style={{ marginLeft: 'auto', textAlign: 'right', flexShrink: 0 }}>
                                        ...
                                        <span style={{ ...mono, display: 'block', fontSize: 'var(--co-readable-small, 0.58rem)', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: col }}>{bandLabel(p.band)}</span>
                                    </span>
                                </div>
                                {factors.length ? (
                                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginTop: '6px' }}>
                                        {factors.map((f, fi) => (
                                            <span key={fi} style={{ fontSize: 'var(--co-readable-small, 0.62rem)', color: MUTED, border: `1px solid ${LINE}`, borderRadius: 'var(--card-radius-xs, 5px)', padding: '1px 6px' }}>{f}</span>
                                        ))}
                                    </div>
                                ) : null}
                                {plays.map((play, pi) => (
                                    <div key={pi} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', marginTop: '8px' }}>
                                        ...
                                        <button onClick={() => doCopy('play:' + p.userId + ':' + pi, play)}
                                            style={{ padding: '4px 10px', background: 'transparent', color: GOLD, border: '1px solid rgba(212,175,55,0.5)', borderRadius: 'var(--card-radius-xs, 5px)', font: '700 var(--co-readable-small, 0.62rem) ' + MONO, letterSpacing: '0.05em', textTransform: 'uppercase', cursor: 'pointer', flexShrink: 0 }}>
