...
(function (root) {
    ...
    function Army({army,campaign,onAction,busy,active,retired = false}) {
        ...
        return <article className={'duat-library-army ' + (active ? 'walking' : retired || army.destroyed ? 'retired' : '')}>
            ...
            <p>{army.rulerRealm || 'A ruler of your faction'}<br/><small>{army.season} {App.DuatMystery?.enabled(campaign)?'ruler origin':'scoring season'}{army.rosteredYear ? ` · Buried in ${army.rosteredYear}` : ''}</small></p>
