...
(function(root){
    ...
    function Preparation({campaign,factionId,lineup,onChange,disabled,previous,data,throughWeek=0}){
        ...
        return <section className="duat-panel duat-weekly-lineup"><div className={'duat-lineup-completion'+(legal?' is-complete':'')} role="status"><strong>{lineup.length} / {required} starters</strong><span>{legal?'Lineup complete':lineup.length<required?'Choose '+(required-lineup.length)+' more':lineup.length>required?'Move '+(lineup.length-required)+' to the bench':'Check the starting positions below'}</span></div><details className="duat-lineup-positions" open={!legal&&lineup.length===required}><summary>Starting positions</summary><p>{E.ROSTERS[rules.roster].slots.join(' · ').replaceAll('SUPER_FLEX','Superflex')}. Your bench rests this week.</p></details><div className="duat-roster">{army?.players.map(player=>{const estimate=E.estimatePlayer(campaign,factionId,player.id),last=previous?.factions.find(item=>item.factionId===factionId)?.players.find(item=>item.id===player.id);return <div key={player.id} className="duat-player-entry"><label className={'duat-player '+(lineup.includes(player.id)?'starting':'')}><input type="checkbox" disabled={disabled} checked={lineup.includes(player.id)} onChange={()=>onChange(lineup.includes(player.id)?lineup.filter(id=>id!==player.id):[...lineup,player.id])}/><span className="duat-position">{player.position}</span><span className="duat-player-name"><strong>{player.name}</strong><small>{App.DuatMystery?.enabled(campaign)?App.DuatMystery.playerLabel(player)+' · '+estimate.label:estimate.label}</small></span><span className="duat-player-points"><strong>{Number(estimate.points||0).toFixed(1)}</strong><small>Est. PPG</small></span>{last&&<span className="duat-player-points"><strong>{Number(last.effectivePoints||0).toFixed(1)}</strong><small>W{previous.week}</small></span>}</label>{App.DuatMysteryUI&&<App.DuatMysteryUI.Explorer campaign={campaign} player={player} data={data} throughWeek={throughWeek} factionId={factionId}/>}</div>;})}</div><button className="duat-link" disabled={disabled} onClick={()=>onChange(E.recommendedLineup(campaign,factionId))}>Suggest a legal lineup</button></section>;
    }
