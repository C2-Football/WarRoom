function WrAllLeaguesWire({ leagues = [], accountId = '', onClose, onOpenLeague }) {
    ...
    const [phone, setPhone] = React.useState(Boolean(window.matchMedia?.('(max-width: 767px)').matches));
    React.useEffect(() => {
    ...
    const coverage = <details className="wr-all-wire-coverage" open={phone ? undefined : true}><summary>Filters & coverage · {ready}/{eligible.length} leagues{partial ? ` · ${partial} partial` : ''}</summary>
    ...
    const story = (s, i) => (<article key={`${s.league.id || s.league.league_id}:${s.id || s.text}`} className={'wr-all-wire-story' + (i === 0 ? ' is-lead' : '')}>
                <div className="wr-journal-kicker"><span>{s.league.name} · {s.league.season}</span></div><p className="wr-all-wire-label">{s.label}</p><h3>{s.text}</h3><div className="wr-journal-body">{s.body?.split(/\n\n+/).map((paragraph, n) => <p key={n}>{paragraph}</p>)}</div>
