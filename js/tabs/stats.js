...
function LeagueStatsTab({ currentLeague, myRoster, playersData, getOwnerName, getPlayerName }) {
    ...
    return <section className={`dhq-stats${isPhone ? ' dhs-mobile' : ''}`} aria-label="Player statistics">
        ...
        <header className="dhs-title"><div>{!isPhone && <h2>Stats</h2>}<p className="dhs-note">{isPhone ? `${season} · ${scopeLabel} · ${perGame ? 'Per game' : 'League scoring'}` : `Every player. Every available stat. Scored for ${currentLeague?.name || 'your league'}.`}</p></div>
        ...
        {isPhone ? <React.Fragment><div className="dhs-mobile-controls">
            <label className="dhs-mobile-search"><input type="search" aria-label="Find a player" placeholder="Search players, teams, positions" value={search} onChange={e => setSearch(e.target.value)} /></label>
        ...
        {isPhone && <LeagueStatsMobileSheet open={!!mobilePanel} title={mobilePanel === 'filters' ? 'Filter players' : 'Stat view'} onClose={() => setMobilePanel(null)}>{mobilePanel === 'filters' ? <React.Fragment><div className="dhs-sheet-fields">{filterFields}</div><div className="dhs-sheet-footer"><button type="button" onClick={() => setMobilePanel(null)}>Show {filtered.length.toLocaleString()} players</button></div></React.Fragment> : <React.Fragment><div className="dhs-sheet-fields"><div className="dhs-mobile-sort"><label>Sort by<select aria-label="Sort players by" value={sort.key} onChange={e => setSort({ key: e.target.value, direction: e.target.value === 'name' ? 1 : -1 })}><option value="name">Player name</option>{allMetrics.map(metric => <option key={metric.key} value={metric.key}>{metric.key === 'fantasyPoints' ? 'Fantasy points' : metric.label}</option>)}</select></label><button type="button" aria-label={sort.direction === 1 ? 'Sort descending' : 'Sort ascending'} onClick={() => setSort(previous => ({ ...previous, direction: -previous.direction }))}>{sort.direction === 1 ? '↑' : '↓'}</button></div>{viewControls}<label>Layout<select value={mobileTable ? 'table' : 'players'} onChange={e => setMobileTable(e.target.value === 'table')}><option value="players">Player rows</option><option value="table">Table · All columns</option></select></label></div>{columnPicker}<button type="button" onClick={exportCsv} disabled={!filtered.length || !data}>Export CSV</button><div className="dhs-sheet-footer"><button type="button" onClick={() => setMobilePanel(null)}>Done</button></div></React.Fragment>}</LeagueStatsMobileSheet>}
