...
RevenueCat `TRANSFER`/cross-account restore semantics remain a separate unfinished existing native journey. Checkout double-submission protection beyond stable Stripe customer creation, current native client billing error/activation wording, and provider-backed repair of already-lost legacy source history also remain outside this verified recovery subset. Preserve these as open work, not optional scope.
