...
Status: the original layout source matrix and final production-build local matrix both passed at all four viewports; each process exited 0. The final compiled run includes the subsequent research-loading copy guard and archive-readiness harness correction. Independent classic historical checks passed at phone and desktop sizes. No production source was edited by this QA agent.
...
The final compiled production candidate passed all four viewport journeys, terminal exit 0, with identical row heights and all accessibility, truthfulness, selection, persistence, safe-area, and pregame checks. Invocation: `DUAT_LINEUP_URL=http://127.0.0.1:3026/dist-preview/ DUAT_LINEUP_PHASE=compiled node tests/duat-lineup-density-browser.cjs`. [Compiled results and source hashes](duat-lineup-density-20260925/compiled-evidence.json), [compiled log](duat-lineup-density-20260925/compiled-browser.log), [compiled phone screenshot](duat-lineup-density-20260925/compiled-lineup-390.png), [compiled short landscape](duat-lineup-density-20260925/compiled-lineup-667.png).

The archived `after-*` artifacts describe the initial layout source shipped as `34a6b4c2b239e7d35425746ec797220ce79261ab`. The archived `compiled-*` artifacts were replaced by the final frozen follow-up candidate after the loading-copy guard was added. Their recorded production hashes identify the exact tested code. The final harness waits for a real 17-row archive table independently of the expected candidate count, retains exact-clue checks, waits for the real kickoff control after reload, and rejects any match/contradiction verdict whenever an actual loading state is observed. The natural initial hosted timing reproduction and its preserved failure are documented in the [hosted evidence](duat-lineup-density-hosted-20260925/README.md); the initial hosted failure is not counted as a pass.
