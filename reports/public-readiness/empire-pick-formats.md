...
Independent root review found one format precedence edge: an empty top-level `type` hid the valid `settings.type` under nullish selection. Reproduced actual canonical `dynasty` versus Empire `unknown`; corrected selection now matches canonical first-nonempty behavior while preserving numeric zero. Added a sixth test comparing actual canonical types for blank dynasty, keeper-count fallback, and explicit redraft. No unresolved root finding remains for this bounded correction.
