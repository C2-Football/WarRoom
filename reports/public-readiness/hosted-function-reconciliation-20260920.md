...
Existing signup provisioning rollback still removes only the account created by that request when its initial subscription insert fails. It is unrelated to QA reset behavior and remains covered by the existing provisioning contract.
