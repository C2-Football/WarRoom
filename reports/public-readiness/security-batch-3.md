...
Independent review: `product_inventory` reran and approved the identity-guard regression. The reviewer found an adjacent reconnect defect: existing `od_auth_v1.sleeperUsername` could override the newly written `username` in the hub. The follow-up uses the provider's canonical username and updates both compatibility fields, locked pointer, profile and cloud metadata coherently. Added regression begins with a different existing connection and verifies all pointers select the newly verified account. Persistence under partial quota failure remains the separately tracked INV-03 rather than a claimed fix.
