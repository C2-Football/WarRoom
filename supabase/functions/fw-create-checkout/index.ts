...
async function handleBillingRequest(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (!await billingProcessingEnabled(admin)) return json(req, { error: 'Billing is temporarily unavailable. Please try again shortly.' }, 503);
    ...
    const safeSuccessUrl = validateCheckoutUrl(successUrl, 'https://dhqfootball.com/upgrade.html?payment=success');
    const safeCancelUrl = validateCheckoutUrl(cancelUrl, 'https://dhqfootball.com/upgrade.html?payment=cancel');
    ...
    const { data: appUser, error: accountError } = await admin
      .from('app_users')
    ...
    if (accountError || !appUser) {
      return json(req, { error: 'Billing account lookup is temporarily unavailable. Try again.' }, 503);
    }
    ...
    if (!customerId) {
      ...
      }, { idempotencyKey: `dhq-customer-v1:${userId}` });
      ...
      const { data: savedCustomer, error: customerError } = await admin
        ...
        .select('id')
        .single();
      if (customerError || !savedCustomer) {
        return json(req, { error: 'Billing account could not be saved. Try again.' }, 503);
      }
    }
...
function allowedCheckoutOrigins(): string[] {
  ...
  const defaults = 'http://localhost:3001,http://localhost:3002,http://127.0.0.1:3001,http://127.0.0.1:3002,https://jcc100218.github.io,https://warroom.skjjcruz.com,https://skjjcruz.github.io,https://dhqfootball.com,https://www.dhqfootball.com,https://c2-football.github.io';
