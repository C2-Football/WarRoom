...
-- Extend only Duat contracts. Versions one and two retain their original rules; version three stores its constitution.
...
begin
    if p_user_id is null or jsonb_typeof(p_state) is distinct from 'object' or (((p_state->>'phase'='preseason' and p_state->>'version'='1') or (p_state->>'phase'='draft' and p_state->>'version' in ('2','3') and p_state#>>'{draft,status}'='waiting')) is not true) then raise exception 'Invalid new campaign'; end if;
    v_size := case when p_state->>'version'='3' then (p_state#>>'{settings,leagueSize}')::integer else 14 end;
...
declare v_room duat_campaigns; v_member duat_campaign_members; v_receipt duat_campaign_actions;
    v_type text; v_ready boolean; v_revision integer; v_cursor integer; v_position integer; v_turn text; v_size integer; v_rounds integer;
begin
    ...
    if v_type='set-ready' then
        ...
        if jsonb_typeof(p_next_state) is distinct from 'object' or p_next_state->>'id' is distinct from v_room.state->>'id'
            ...
            or p_next_state->'seasons' is distinct from v_room.state->'seasons'
