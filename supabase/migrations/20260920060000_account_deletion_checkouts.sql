-- Extend the reviewed deletion inventory after checkout recovery migration 050000.
-- Claim/checkpoint checkout writers lock the same app row; the final snapshot
-- compares complete attempts and customer binding before cascading them.
begin;
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(c) order by c.product_slug),'[]'::jsonb)
    into v_checkouts from public.billing_checkout_attempts c where c.user_id=v_target.id;
  return jsonb_build_object(
    'actor',jsonb_build_object('id',v_actor.id,'session_version',v_actor.session_version),
    'target',case when v_target.id is null then null else jsonb_build_object('id',v_target.id,'email',v_target.email,'session_version',v_target.session_version,'stripe_customer_id',v_target.stripe_customer_id) end,
    'email',p_email,'self',p_self,'subscriptions',v_subscriptions,'sources',v_sources,'checkout_attempts',v_checkouts);
end;
$$;
...
begin
  ...
  if v_id is not null then
    if exists(select 1 from public.billing_checkout_attempts where user_id=v_id and lease_expires_at>clock_timestamp()) then
      raise exception 'Checkout is still running; retry deletion' using errcode='40001';
    end if;
