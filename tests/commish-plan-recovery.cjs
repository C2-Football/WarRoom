...
const proposalCallbacks = source.slice(source.indexOf('    const PROPOSALS_KEY ='), source.indexOf('    // Ratify:'));
...
try {
    ...
    rejected = false; assert.equal(actions.onDeleteProposal(list[0].id), true); ({ ui, actions } = render()); assert.equal(ui.failure, null); assert.equal(storage.get('commish_rulelab_proposals', []).length, 0);
