...
let hooks = [], index = 0, board, liveContext, requestedWeek;
...
const React = {
    ...
    createElement(type, props, ...children) { return typeof type === 'function' && type !== boundary ? type({ ...(props || {}), children }) : { type, props: props || {}, children }; }
};
...
function render() { index = 0; return root.LeagueCentralTab(props); }
...
function table(tree) { return find(tree, n => n.type === 'table' && /standings/.test(n.props['aria-label'] || '')); }
function teamRows(tree) { const body = find(table(tree), n => n.type === 'tbody'); return walk(body).filter(n => n.type === 'tr'); }
...
assert(teamRows(tree).every(row => !find(row, n => n.props['data-result'] === 'T')), 'Pregame zero scores do not create phantom ties');
