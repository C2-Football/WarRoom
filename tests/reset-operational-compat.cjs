...
const cors=load('supabase/functions/_shared/cors.ts',{Deno:deno}).context;
for(const origin of ['https://dhqfootball.com','https://www.dhqfootball.com','https://c2-football.github.io','https://jcc100218.github.io','https://skjjcruz.github.io','capacitor://localhost','https://localhost','https://configured.example.invalid']) {
 ...
 assert.equal(cors.corsHeaders(request)['Access-Control-Allow-Origin'],origin);
}
for(const module of [security,cors])assert.notEqual(module.corsHeaders(new Request('https://example.invalid',{headers:{Origin:'https://untrusted.example.invalid'}}))['Access-Control-Allow-Origin'],'https://untrusted.example.invalid');
...
console.log('PASS actual shared/proxy CORS preserve supported web/native origins plus configured origins without wildcard; reserved-address helper retained');
