...
async function run() {
    ...
    const artwork = { ...App.TimeLeagueHelmet.defaultHelmet('Original'), assetId: 'simanek', artworkMode: 'original' };
    nodes(view, node => node.type === window.TimeLeagueHelmetPicker)[0].props.onChange(artwork);
    view = render();
    nodes(view, node => node.type === 'input' && node.props['aria-label'] === 'Primary color')[0].props.onChange({ target: { value: '#123456' } });
    view = render();
    let chosen = nodes(view, node => node.type === window.TimeLeagueHelmetPicker)[0].props.helmet;
    assert.equal(chosen.assetId, 'simanek');
    assert.equal(chosen.artworkMode, 'original', 'Changing a profile color must preserve the artist color mode');
    assert.equal(chosen.shellColor, artwork.shellColor, 'Profile colors must not silently recolor the helmet');
    nodes(view, node => node.type === 'button' && text(node) === 'Use team colors on helmet')[0].props.onClick();
    view = render();
    chosen = nodes(view, node => node.type === window.TimeLeagueHelmetPicker)[0].props.helmet;
    assert.equal(chosen.assetId, 'simanek', 'Applying team colors must retain the chosen complete asset');
    assert.equal(chosen.artworkMode, 'team-colors');
    assert.equal(chosen.shellColor, '#123456');
    userId = 'alice';
