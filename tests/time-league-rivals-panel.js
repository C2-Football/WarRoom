...
(async () => {
    ...
    const realNow = Date.now, realTimeout = window.setTimeout, realClear = window.clearTimeout;
    let now = realNow(); const timers = new Map(); let timerId = 0;
    Date.now = () => now;
    window.setTimeout = (fn, delay) => { const id = ++timerId; timers.set(id, { fn, delay }); return id; };
    window.clearTimeout = id => timers.delete(id);
    league = { ...league, leagueId: 'typing-delay-test', rivalMessages: [] };
    league = R.sendMessage(league, { teamId: 'you', toTeamId: 'ai', messageId: 'typing_test_01', text: 'Ready for our next game?', tone: 'friendly' }, new Date(now).toISOString());
    const reply = league.rivalMessages.find(message => message.id === 'reply:typing_test_01');
    let presented = Mail.presentation(league);
    assert.equal(league.rivalMessages.length, 2, 'Both messages are already saved during the visual delay');
    assert(!Mail.unreadMessages(presented.league, 'you', []).some(message => message.id === reply.id), 'Unread notices cannot expose a typing reply');
    tree = render(); tree = render(); owner(tree, 'Kade').props.onClick(); tree = render();
    assert(text(tree).includes('Ready for our next game?'), 'The outgoing message appears immediately');
    assert(all(tree, node => node.props['aria-label'] === 'Kade is typing').length === 1);
    assert(!text(tree).includes(reply.text), 'The reply and preview stay hidden while dots are showing');
    assert(!Mail.readIds(league.leagueId, 'you').includes(reply.id), 'Hidden replies cannot be marked read');
    owner(tree, 'Friend').props.onClick(); tree = render();
    assert.equal(all(tree, node => node.props.className === 'tl-chat-typing').length, 0, 'Typing belongs to the correct conversation only');
    owner(tree, 'Kade').props.onClick(); tree = render();
    now += 3000; for (const timer of [...timers.values()]) timer.fn(); timers.clear(); tree = render(); tree = render();
    assert.equal(all(tree, node => node.props.className === 'tl-chat-typing').length, 0);
    assert(text(tree).includes(reply.text), 'The reply replaces the dots after the timer fires');
    assert(Mail.readIds(league.leagueId, 'you').includes(reply.id), 'The visible reply is now read');
    assert.equal(Mail.presentation(league).typing.length, 0, 'Reopening saved history does not replay typing');
    const humanLeague = R.sendMessage(league, { teamId: 'you', toTeamId: 'friend', messageId: 'typing_human_01', text: 'Hello', tone: 'neutral' }, new Date(now).toISOString());
    assert.equal(Mail.presentation(humanLeague).typing.length, 0, 'Human correspondence never gets simulated typing');
    Date.now = realNow; window.setTimeout = realTimeout; window.clearTimeout = realClear;
